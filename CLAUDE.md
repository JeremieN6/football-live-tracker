...
**Phase** : Phase A multi-club terminee et validee en prod. Phase B (RLS par equipe) ecrite, migration a appliquer.
**Derniere session** : 11/09/2026
**Progression globale** : 100% MVP + effectif/rapport nominatif/profil joueur/multi-club Phase A, tout teste de bout en bout ; Phase B (securite par equipe) prete cote code
...
- [x] Phase B multi-club (code pret, migration a appliquer) — Policies RLS de `players`/`matches`/`events`/`match_lineups`/`reports` reecrites pour un cloisonnement reel par equipe : un OWNER voit/gere tout son club, un COACH est limite a sa propre equipe (`team_id`). Exception volontaire : la lecture de `players` reste club-wide (tout membre actif voit tous les joueurs, y compris ceux d'une autre equipe) pour que les noms des joueurs "appeles en renfort" restent lisibles dans les matchs/rapports d'une autre equipe — seule l'ecriture (creer/modifier/archiver) reste limitee a l'equipe du joueur. Deux fonctions SQL `security definer` (`can_access_team`, `is_club_member`) evitent la duplication de logique et la recursion RLS. Les anciennes policies de ces 5 tables (ecrites avant ce projet, noms inconnus) sont supprimees dynamiquement via `pg_policies` avant de recreer les nouvelles. Les policies SELECT de `clubs`/`teams` sont aussi elargies a tout membre actif (pas que le OWNER), pour ne pas avoir a re-toucher RLS au moment de la Phase C.

### Prochaines etapes :
- [ ] Appliquer la migration `20260911200000_phase_b_team_rls.sql` (Phase B) sur le projet Supabase reel, puis retester le parcours complet pour s'assurer que rien n'a casse pour le OWNER actuel (qui doit continuer a tout voir/gerer normalement)
- [ ] Phase C multi-club : invitations par email (le OWNER invite un coach sur une equipe precise via `club_members`, table deja prete avec `status PENDING/ACTIVE` et `invited_email`), UI de gestion des membres. Attention : `clubsStore.ensureClub()` cote client suppose aujourd'hui que l'utilisateur est proprietaire (cree un nouveau club s'il n'en trouve pas) — a adapter en Phase C pour un COACH invite qui n'est pas OWNER (chercher d'abord dans `club_members`, ne pas creer de club automatiquement dans ce cas)

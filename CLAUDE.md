...
### Prochaines etapes :
- [ ] Appliquer la migration `20260911180000_fix_club_members.sql` (la table `club_members` n'existait pas du tout en prod — migration 140000 arretee avant la fin — cette migration la cree, recree les 12 policies clubs/teams/club_members, et backfill la ligne OWNER manquante) puis reessayer de creer une equipe dans `/teams`. Remplace `20260911170000_recreate_club_policies.sql` (supprimee, n'a jamais pu s'appliquer puisqu'elle supposait club_members deja existante).
